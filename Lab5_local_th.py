...
adjusted_gaussian_img = np.zeros((10,10,3), dtype=np.uint8)  #* Global Threshold GaussianWeightSum-c algorithm
...
def handler_adjustThreshold(x):
    ...
    #* box size or window_size
    window_size = (cv.getTrackbarPos('window_size','Binary') * 2 ) + 1 
    print(f"Global Threshold = {threshold_value}")
    print(f"window_size = {window_size}")
    print(f"c_value = {c_value}")
    # ! อธิบาย
    # -----------------------------
    # _, adjusted_img = cv.adaptiveThreshold(source_img,255,cv.ADAPTIVE_THRESH_GAUSSIAN_C,\
    ...
    # ! เปลี่ยนเป็น cv.adaptivethreshold 
    # ! อธิบายแค่คำสั่ง threshold #ค่า max 255 เป็น ขาว 0 เป็นขาว อัลกอริทึมเราใช้เป็น THRESH_BINARY 
    # Update histogram
    histSize = 256
...
def main():
    global threshold_value,window_size,c_value,adjusted_mean_img,adjusted_gaussian_img
    global source_img,adjusted_img,hist_img
    if(len(sys.argv)>=2):
        source_img = cv.imread(str(sys.argv[1]))
    else :
        source_img = cv.imread("./sudoko.png", 1)
